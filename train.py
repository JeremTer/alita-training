...
def train_single_epoch(model, data_loader, loss_fn, optimiser, device):
    for input, target in data_loader:
        ...
        #loss = F.nll_loss(prediction.squeeze(), target)
...
if __name__ == "__main__":
    ...
    optimiser = torch.optim.Adam(m5.parameters(),
                                 lr=LEARNING_RATE)

    # train model
    train(m5, train_dataloader, loss_fn, optimiser, device, EPOCHS)

    countSame = 0
    for i in range(len(asd)):
        signal, label = asd[i]
        result = predict(m5, signal)
